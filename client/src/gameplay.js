...
const state = { //tracks global information 
...
function moveGroup(group, deltaX, deltaZ) { //moves group of connected pieces by delta 
...
function returnPositionalErrorAndSomeOtherStuff(mesh, slotName, expectedPartner) { //calculates the difference between actual displacement and expected displacement between a piece and its partner 
...
function checkIfPieceSnaps(mesh, droppedGroup) { //checks if two pieces are close enough to snap 
    for(const [slotName, slotData] of Object.entries(mesh.userData.slots)){  //first checks for held piece 
        if (!slotData.isConnected && slotData.partnerPiece) {
            ...
            if (result.errorMagnitude < config.snapThreshold){ //if success, returns mesh, slot of mesh, mesh to be snapped to, and the deltas to calculate snap 
                return { 
                    mesh: mesh,
                    slotName, 
    ...
    for (const piece of droppedGroup){ //else iterates through the rest of the pieces in the group 
        ...
        for(const [slotName, slotData] of Object.entries(piece.userData.slots)){
            if (!slotData.isConnected && slotData.partnerPiece) {
                ...
                if (result.errorMagnitude < config.snapThreshold){ //does the same as earlier except with a different mesh 
                    return { 
                        mesh: piece,
                        slotName, 
                        partnerPiece: partner, 
...
function checkIfPieceConnects(group) {  //checks if two pieces are close enough to snap (smaller threshold than snap threshold because this is operating under the assumption that the pieces are already where they need to be, with the threshold just for floating point stuff 
    for (const piece of group) {
        for (const [slotName, slotData] of Object.entries(piece.userData.slots)) {
...
function mergeGroups(group1, group2) { //updates the groups of two connecting groups to update the state of the new merged group 
...
function handleSnapping(heldPiece, droppedGroup) { //handles everything snapping related when you drop a piece 
...
function onPointerMove(event) { //updates cursor position in normalized device coordinates 
    state.cursor.x = (event.clientX / window.innerWidth) * 2 - 1
    state.cursor.y = -(event.clientY / window.innerHeight) * 2 + 1
}

function onMouseDown(event, raycaster, camera, controls) { //handles selecting an object on mouse down 
    if(event.button !== 0) return //if it wasnt a left mouse input 

    if (state.hoveredObject !== null) { //if click was done without a hovered object 
        ...
        if (intersects.length > 0) { //checks if there is a object under cursor and if so makes that the selected object . now that i think about it you could probably do without this chekc but whatever mane
            ...
            state.dragPlane.setFromNormalAndCoplanarPoint( //calculates the plane that the mesh is on. also kinda redundant atp but maybe ill implement floating pieces and stuff later
...
function onMouseUp(event, controls) { //handles dropping an object on mouse down 
...
function updateDrag(raycaster, camera) {  //handles updating group positions when dragging 
...
function updateHover(raycaster, camera, scene) {  //handles updating hovered object state  
...
export function setupGameplay(camera, scene, controls, raycaster) { //sets up all the gameplay logic 
