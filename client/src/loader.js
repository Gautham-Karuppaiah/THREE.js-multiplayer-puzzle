    ...
    return new Promise((resolve) => {
        ...
        gltfLoader.load("/models/JIGSAW_PIECES_FINAL.glb", (gltf) => {
            ...
            for (let i = 0; i < children.length; i++) {
                //iterate through children in the scene
                ...
                if (child.isMesh) {
                    ...
                    const tabMask = name
                        .split("")
                        .map((d) => (d === "1" ? 1 : 0))
                        .join("") //gets the piece permutation via the file name. from top clockwise, 1= tab out, 2= straight edge, 0 = tab in

                    const uvConfig = {
                        //determines hashmap for correcting uv textures. blender exports the uvmaps squishing the non-square pieces into a square, so they have to be unsquished based on which sides have a tab sticking out
                    ...
                    const [pivotPointX, pivotPointY, enableScaleX, enableScaleY] = uvConfig[
                        tabMask
                    ] || [0.5, 0.5, true, true]
